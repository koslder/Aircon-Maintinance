...
import { DatePicker, Modal, Button, Layout, Menu, Table, Popconfirm, Input, Form, Flex } from "antd";
...
const AdminPanel = () => {
    ...
    const handleUpdate = async (id) => {
        const adminToUpdate = admins.find((admin) => admin._id === id);
        if (adminToUpdate) {
            setCurrentAdmin(adminToUpdate);
            form.setFieldsValue({
                ...adminToUpdate,
                birthdate: dayjs(adminToUpdate.birthdate),
            });
            updateModal();
        }
    };
    ...
    const handleAddSubmit = async (values) => {
        try {
            ...
            console.log("Submitting the following data:", formattedValues); // Log the formatted data
            ...
            setAdmins((prevAdmins) => [...prevAdmins, response.data]); // Add new admin to state
            closeAddModal();
        } catch (error) {
    ...
    const handleFormSubmit = async (values) => {
        try {
            ...
            console.log("Updating the following data:", formattedValues); // Log the data to be updated

            await axios.patch(`https://capstone-sever.onrender.com/api/admins/${currentAdmin._id}`, formattedValues);

            // Update state to reflect the updated admin
            setAdmins((prevAdmins) =>
                prevAdmins.map((admin) =>
                    admin._id === currentAdmin._id ? { ...admin, ...formattedValues } : admin
                )
            );
            closeModal();
        } catch (error) {
